...
def _simulate(w1, n_trials, n_t, seed):
    np.random.seed(seed)
    truth = StratifiedLinear(tau=10.0, process_noise=0.1, measure_noise=0.05, w1=w1)
...
def test_fit_recovers_linear_weight():
    ...
    m = StratifiedLinear(tau=10.0, process_noise=0.1, measure_noise=0.05)
...
def test_fit_latentseries_matches_dicts():
    ...
    m1 = StratifiedLinear(tau=10.0, process_noise=0.1, measure_noise=0.05)
    m1.fit(states, inputs, **_FIT)
    m2 = StratifiedLinear(tau=10.0, process_noise=0.1, measure_noise=0.05)
